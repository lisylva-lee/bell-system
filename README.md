...
| **🌐 Web 远程控制** | 局域网内手机/电脑浏览器远程管理：任务增删改查、立即打铃、暂停、切换方案、查看日志（v2.0） |
...
4. **远程控制**：系统设置 → 勾选「启用局域网网页远程控制」→ 手机浏览器打开主界面底部状态栏显示的地址即可管理（可选设置访问令牌）
...
### v2.0 (2026-08-22)
- 🌐 新增：Web 远程控制 —— 软件内置 HTTP 服务，同一局域网的手机/电脑用浏览器即可管理打铃系统
- ✅ 远程能力：实时状态与时钟、下次打铃倒计时、任务添加/编辑/删除/启用开关、单任务试听、立即打铃（全部/单个）、停止播放、暂停/恢复、方案切换、执行日志
- ✅ 移动端优先网页（自适应手机屏幕），数据自动轮询刷新
- ✅ 安全：可选访问令牌（Token）防局域网误操作；端口可配置（默认 8787）
- ✅ 零新增依赖：服务端仅用 Python 标准库 `http.server`；网页内嵌源码打包进 exe，保持单文件绿色版
- 🔧 加固：SQLite 启用 WAL + 写操作线程锁（GUI/调度/Web 三线程并发安全）；调度器时间解析兼容 `HH:MM`
- 🖥 集成：系统设置新增「Web 远程控制」区块；主界面底部状态栏显示访问地址；托盘菜单新增「打开网页控制台」
